...
const description = computed(() => {
  return {
    503: 'Lo sentimos, estamos haciendo mantenimiento a la plataforma. Por favor vuelva mas tarde.',
    500: '¡Ooops!, algo fue mal en nuestros servidores, estamos trabajando para solucionarlo a la brevedad.',
    404: 'Lo sentimos, no ha sido posible encontrar la pagina que estas buscando.',
    403: 'Lo sentimos, no tienes permisos para acceder a esta pagina',
  }[props.status]
})
</script>
...
<template>
    ...
    <div class="w-full min-h-screen flex flex-col lg:flex-row items-center justify-center space-y-16 lg:space-y-0 space-x-8 2xl:space-x-0">
        <div class="w-full lg:w-1/2 flex flex-col items-center justify-center lg:px-2 xl:px-0 text-center">
            <p class="text-7xl md:text-8xl lg:text-9xl font-bold tracking-wider text-gray-300">{{ props.status ?? 500 }}</p>
            <p class="text-4xl md:text-5xl lg:text-6xl font-bold tracking-wider text-gray-300 mt-2">Pagina no encontrada</p>
            <p class="text-lg md:text-xl lg:text-2xl text-gray-500 my-12">{{ description }}</p>
            <!-- @auth -->
            <div v-if="$page.props.user">

                <p class="text-lg md:text-xl lg:text-2xl text-gray-500">{{ $page.props.user.name }}</p>
                <div v-if="$page.props.user?.all_teams.length > 0">
                    <p class="text-lg text-gray-500">perteneces a {{ $page.props.user?.all_teams.length }} agencias.</p>
                    <p class="text-lg text-gray-500 my-4">
                        Ir a la agencia 
                    </p>
                    <div v-for="$team in $page.props.user.all_teams">

                        <Link :href="route('team.current.switch', {'team_id': $team.id})" class="text-xl font-bold text-sky-500 inline" method="PUT">
                            Ir a la agencia {{ $team.name }}
                        </Link>

                    </div>
                </div>
                <div v-else>
                    <p class="text-lg text-gray-500 my-6">¡Ups! parece que no perteneces a ninguna agencia aun, unete por medio de una invitación.</p>
                </div>
