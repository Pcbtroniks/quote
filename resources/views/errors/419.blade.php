    ...
    <body class="font-sans antialiased relative">
        ...
        @if ($errors->any())
            ...
            @foreach ($errors->all() as $error)
                <h1 class="text-xl text-red-500">{{ $error }}</h1>
            @endforeach
        ...
        <div class="w-full min-h-screen flex flex-col lg:flex-row items-center justify-center space-y-16 lg:space-y-0 space-x-8 2xl:space-x-0">
            <div class="w-full lg:w-1/2 flex flex-col items-center justify-center lg:px-2 xl:px-0 text-center">
                <p class="text-7xl md:text-8xl lg:text-9xl font-bold tracking-wider text-gray-300">419</p>
                <p class="text-4xl md:text-5xl lg:text-6xl font-bold tracking-wider text-gray-300 mt-2">Soy una tetera</p>
                <p class="text-lg md:text-xl lg:text-2xl text-gray-500 my-12">Lo sentimos, ha ocurrido un evento excepcional.</p>
                @auth
                    <p class="text-lg md:text-xl lg:text-2xl text-gray-500">{{ auth()->user()->name }}</p>
                    @if (count(auth()->user()->allTeams()) >= 1)
                        <p class="text-lg text-gray-500">perteneces a {{ count(auth()->user()->allTeams()) }} agencias.</p>
                        <p class="text-lg text-gray-500 my-4">
                            Ir a la agencia 
                        </p>
                        @foreach (auth()->user()->allTeams() as $team)
                            <form class="my-2" method="POST" action="{{ route('team.current.switch', ["team_id" => $team->id]) }}">
                                <button type="submit" class="text-xl font-bold text-sky-500 inline">Agencia {{ $team->name }}</button>
                                @method('PUT')
                                @csrf
                            </form> 
                        @endforeach
